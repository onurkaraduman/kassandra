...
public class NewsIntegrationServiceWrapper {
    ...
    public Call<List<News>> getNewsByModifiedDate(Date date) {
        DateTime startDate = new DateTime(date);
        DateTime endDate = new DateTime(DateUtil.oneDayAfter(date));
        return getNewsByModifiedDate(DateUtil.DATE_FORMATTER2.print(startDate), DateUtil.DATE_FORMATTER2.print(endDate));
    }
